flectra.define('kitchen_screen.update_kanban', function (require) {
    'use strict';
    ...
    KanbanRecord.include({
        ...
        _openRecord: function () {
            if (this.modelName === 'kitchen.screen' && this.$(".o_kitchen_screen_kanban_boxes a").length) {
                this.$('.o_kitchen_screen_kanban_boxes a').first().click();
            } else {
        ...
        _onKanbanActionClicked: function (ev) {
            var self = this;
            if (this.modelName === 'kitchen.order' && $(ev.currentTarget).data('type') === 'set_cover') {
                ev.preventDefault();
    
                var domain = [['res_model', '=', 'kitchen.order'], ['res_id', '=', this.id], ['mimetype', 'ilike', 'image']];
                this._rpc({
                        model: 'ir.attachment',
                        method: 'search_read',
                        domain: domain,
            ...
            } else if (this.modelName === 'kitchen.screen' && $(ev.currentTarget).data('type') === 'set_cover') {
                ev.preventDefault();
    
                domain = [['res_model', '=', 'kitchen.screen'], ['res_id', '=', this.id], ['mimetype', 'ilike', 'image']];
                this._rpc({
                        model: 'ir.attachment',
                        method: 'search_read',
                        domain: domain,
